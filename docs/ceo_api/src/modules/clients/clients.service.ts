...
export class ClientsService {
  ...
  async create(tenantId: number, dto: CreateClientDto) {
    ...
    const existing = await pool
      ...
      .query(`SELECT id FROM [client] WHERE code = @code AND deleted_at IS NULL`);
    ...
    const result = await pool
      ...
      .input('clientType', sql.NVarChar, dto.clientType || null)
      ...
      .query(`
        INSERT INTO [client] (
          code, name, tax_id, client_type, segment, status, rating,
          credit_limit, payment_terms, notes, company_id, created_at
        )
        OUTPUT INSERTED.id
        VALUES (
          @code, @name, @taxId, @clientType, @segment, @status, @rating,
          @creditLimit, @paymentTerms, @notes, @companyId, GETDATE()
        )
      `);
  ...
  /**
   * Find all clients with optional filtering and pagination
   */
  async findAll(
    tenantId: number,
    filters: {
    ...
    if (filters.clientType) {
      conditions.push('c.client_type = @clientType');
    }
    ...
    if (filters.searchText) {
      conditions.push('(c.name LIKE @searchText OR c.code LIKE @searchText OR c.tax_id LIKE @searchText)');
    }
    ...
    if (!filters.page || !filters.pageSize) {
      ...
      if (filters.clientType) request.input('clientType', sql.NVarChar, filters.clientType);
      ...
      const result = await request.query(`
        ...
        FROM [client] c
        LEFT JOIN [company] co ON c.company_id = co.id AND co.deleted_at IS NULL
        ${whereClause}
        ORDER BY c.name ASC
      `);
    ...
    if (filters.clientType) countRequest.input('clientType', sql.NVarChar, filters.clientType);
    ...
    if (filters.clientType) dataRequest.input('clientType', sql.NVarChar, filters.clientType);
  ...
  /**
   * Find a client by ID with all related data
   */
  async findOne(tenantId: number, id: number) {
    ...
    // Get contacts
    const contacts = await this.findClientContacts(tenantId, id);

    // Get addresses
    const addresses = await this.findClientAddresses(tenantId, id);
  ...
  async update(tenantId: number, id: number, dto: UpdateClientDto) {
    ...
    const existing = await pool
      .request()
      .input('id', sql.Int, id)
      .query(`SELECT id FROM [client] WHERE id = @id AND deleted_at IS NULL`);
    ...
    if (dto.code) {
      const duplicate = await pool
        ...
        .input('id', sql.Int, id)
        .query(
          `SELECT id FROM [client] WHERE code = @code AND id != @id AND deleted_at IS NULL`,
        );
    ...
    if (dto.clientType !== undefined) {
      updates.push('client_type = @clientType');
      request.input('clientType', sql.NVarChar, dto.clientType);
    }
    ...
    if (dto.creditLimit !== undefined) {
      updates.push('credit_limit = @creditLimit');
  ...
  async block(tenantId: number, id: number, reason: string) {
    ...
    const clientResult = await pool
      .request()
      .input('id', sql.Int, id)
      .query(`SELECT notes FROM [client] WHERE id = @id AND deleted_at IS NULL`);

    if (clientResult.recordset.length === 0) {
      throw new NotFoundException('Client not found');
    }

    const currentNotes = clientResult.recordset[0].notes || '';
  ...
  async getStatistics(tenantId: number) {
    ...
    const result = await pool.request().query(`
      SELECT
        ...
        SUM(CASE WHEN status = 'blocked' THEN 1 ELSE 0 END) as blockedClients,
        SUM(CASE WHEN client_type = 'individual' THEN 1 ELSE 0 END) as individualClients,
        SUM(CASE WHEN client_type = 'corporate' THEN 1 ELSE 0 END) as corporateClients,
        SUM(CASE WHEN MONTH(created_at) = MONTH(GETDATE()) AND YEAR(created_at) = YEAR(GETDATE()) THEN 1 ELSE 0 END) as clientsThisMonth
      FROM [client]
   ...
   */
  async findClientContacts(tenantId: number, clientId: number) {
    const pool = await this.databaseService.getTenantConnection(tenantId);

    const result = await pool.request().input('clientId', sql.Int, clientId).query(`
      SELECT *
      FROM [contact]
   ...
   */
  async createContact(tenantId: number, clientId: number, contactData: any) {
    const pool = await this.databaseService.getTenantConnection(tenantId);
    ...
    const result = await pool
      .request()
      .input('entityType', sql.NVarChar, 'client')
      .input('entityId', sql.Int, clientId)
   ...
   */
  async findClientAddresses(tenantId: number, clientId: number) {
    const pool = await this.databaseService.getTenantConnection(tenantId);
   ...
   */
  async createAddress(tenantId: number, clientId: number, addressData: any) {
    const pool = await this.databaseService.getTenantConnection(tenantId);
  ...
  /**
   * Verify client by code or email (public access)
   */
  async verifyClient(tenantId: number, dto: { clientCode?: string; email?: string }) {
    if (!dto.clientCode && !dto.email) {
      throw new Error('Must provide either clientCode or email');
    }
    ...
    // If client code is provided, search by code
    if (dto.clientCode) {
      const result = await pool
        .request()
        .input('code', sql.NVarChar, dto.clientCode)
        .query(`
          SELECT id, code, name
          FROM [client]
      ...
      if (result.recordset.length > 0) {
        clientResult = result.recordset[0];
      }
    }

    // If email is provided and no client found yet, search by email
    if (dto.email && !clientResult) {
      // First check contact table for email associated with client
      const contactResult = await pool
        ...
        .query(`
          ...
          FROM [contact] ct
          INNER JOIN [client] c ON ct.entity_id = c.id
          WHERE ct.entity_type = 'client'
            AND ct.contact_type = 'email'
            AND ct.contact_value = @email
            AND ct.deleted_at IS NULL
            AND c.deleted_at IS NULL
        `);

      if (contactResult.recordset.length > 0) {
        clientResult = contactResult.recordset[0];
      } else {
        // If not found in contacts, check user table for email associated with client
        ...
        if (userResult.recordset.length > 0) {
          clientResult = userResult.recordset[0];
        }
      }
    }
